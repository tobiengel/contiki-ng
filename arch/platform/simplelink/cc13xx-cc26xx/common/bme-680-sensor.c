...
/*---------------------------------------------------------------------------*/
#ifndef Board_BMP680_ADDR
#error "Board file doesn't define I2C address Board_BMP680_ADDR"
#endif
/* Sensor I2C address */
#define BMP680_I2C_ADDRESS                  Board_BMP680_ADDR
/*---------------------------------------------------------------------------*/
/* Registers */
...
/*---------------------------------------------------------------------------*/
static BMP_680_Calibration calib_data;
/*---------------------------------------------------------------------------*/
...
notify_ready(void *unused)
{
  ...
  sensors_changed(&bmp_680_sensor);
}
/*---------------------------------------------------------------------------*/
static bool
i2c_write_read(void *writeBuf, size_t writeCount, void *readBuf, size_t readCount)
{
  I2C_Transaction i2cTransaction = {
    ...
    .readBuf = readBuf,
    .readCount = readCount,
    .slaveAddress = BMP680_I2C_ADDRESS,
  };
...
{
  BMP_680_Calibration *p = &calib_data;
 ...
 * \param type Parameter of type BMP_680_SENSOR_TYPE, choosing between either
...
value(int type)
{
  ...
  switch(type) {
  case BMP_680_SENSOR_TYPE_TEMP:
  case BMP_680_SENSOR_TYPE_PRESS:
    ...
    if(type == BMP_680_SENSOR_TYPE_TEMP) {
      return (int)temp;
    } else if(type == BMP_680_SENSOR_TYPE_PRESS) {
      return (int)pres;
    } else {
      return 0;
    }
...
/*---------------------------------------------------------------------------*/
/**
 * \brief        Configuration function for the BMP680 sensor.
...
/*---------------------------------------------------------------------------*/
SENSORS_SENSOR(bmp_680_sensor, "BMP680", value, configure, status);
/*---------------------------------------------------------------------------*/
